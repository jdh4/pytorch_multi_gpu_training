...
The indices of the GPUs on each node of your allocation begin at 0 and go to N - 1, where N is the total number of GPUs on a node. Consider the case of 2 nodes and 8 tasks with 4 GPUs per node. The process ranks will be 0, 1, 2, 3, 4, 5, 7 while the GPU indices will be 0, 1, 2, 3 on node 1 and 0, 1, 2, 3 on node 2. One cannot make calls such as `data.to(rank)` since this will fail on the second node. Because of this, one cannot use the `rank` as the GPU index for multinode jobs. To deal with this a local rank is introduced:
...
The `local_rank` shoud be used everywhere in your script except when initializing the DDP process group where `rank` should be used. In Python, one uses the  `//` operator for integer division. For exampe, `1 / 2 = 0.5` while `1 // 2 = 0`.
